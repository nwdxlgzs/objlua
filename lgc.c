...
*/
/*
** 该屏障用于推进回收器（collector）的工作，即将黑色对象 'o' 所指向的白色对象 'v' 标记为存活。
** 在分代模式（generational mode）下，若 'o' 是老年对象（old），则 'v' 也必须晋升为老年对象；
** 然而，'v' 不能直接变为 OLD（常规老年状态），因为它可能仍指向非老年对象。
** 因此，'v' 会被标记为 OLD0 状态。在下一个回收周期中，它将变为 OLD1，
** 最终在后续周期中成为 OLD（常规老年对象）。届时，它所指向的所有对象也将成为老年对象。
** 若在增量清除阶段（incremental sweep phase）调用此屏障，则会将黑色对象 'o' 清除为白色（即直接清除），
** 以避免对此对象再次触发屏障操作。
** （此操作在分代模式下不可行，因为分代模式的清除阶段不区分存活白色对象和死亡对象。）
*/
...
*/
/*
** 该屏障用于回退回收器（collector）的工作状态，即将指向白色对象的黑色对象重新标记为灰色。
** （当黑色对象 'o' 指向白色对象 'v' 时，强制将 'o' 从黑色降级为灰色，确保回收器能重新扫描 'o' 的引用关系）
*/
