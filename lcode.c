...
static void codeof(FuncState *fs, BinOpr opr, expdesc *e1, expdesc *e2) {
    int r1 = luaK_exp2anyreg(fs, e1);
    int r2 = luaK_exp2anyreg(fs, e2);
    freeexps(fs, e1, e2);
    if (opr == OPR_TYPEOF) {
        e1->u.info = condjump(fs, OP_TYPEOF, r1, r2, 0, 0);
    } else {
        e1->u.info = condjump(fs, OP_INSTANCEOF, r1, r2, 0, 0);
    }
    e1->k = VJMP;
}
